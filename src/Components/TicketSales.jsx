...
export default function TicketSales() {
  ...
  const filteredSales = sales.filter(sale =>
    ...
    (sale.ticketCategory || '').toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const totalTickets = sales.reduce((sum, s) => sum + (s.ticketCount || 0), 0);
    ...
    <div className="event-management" style={{ background: '#f8fafc', padding: '2rem' }}>
      <div className="event-header" style={{ marginBottom: '1.5rem' }}>
        <h1 className="event-page-title" style={{ fontSize: '1.8rem', fontWeight: 600 }}>
          <MdConfirmationNumber style={{ verticalAlign: 'middle', marginRight: 8 }} />
          Ticket Sales
        </h1>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        <div style={{ background: 'white', borderRadius: 12, padding: '1.25rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <div style={{ color: '#64748b', fontSize: '0.85rem', marginBottom: 4 }}>Total Sales</div>
          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#0f172a' }}>{sales.length}</div>
        </div>
        <div style={{ background: 'white', borderRadius: 12, padding: '1.25rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <div style={{ color: '#64748b', fontSize: '0.85rem', marginBottom: 4 }}>Total Tickets Sold</div>
          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#0a5db8' }}>{totalTickets}</div>
        </div>
        <div style={{ background: 'white', borderRadius: 12, padding: '1.25rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
      ...
      <div className="event-search-section">
        <div className="event-search-wrapper">
          ...
          <input
            ...
            type="text"
            placeholder="Search by event, buyer, or category..."
            value={searchTerm}
      ...
      {/* Sales Table */}
      {loading ? (
        <p style={{ textAlign: 'center', padding: '2rem', color: '#64748b' }}>Loading ticket sales...</p>
      ) : error ? (
        ...
        <div style={{ background: 'white', borderRadius: 12, padding: '3rem', textAlign: 'center', color: '#64748b' }}>
          {sales.length === 0 ? 'No ticket sales yet.' : 'No matching sales found.'}
        </div>
      ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: '#475569' }}>Event</th>
                ...
                <th style={{ padding: '12px 16px', textAlign: 'center', fontWeight: 600, color: '#475569' }}>Qty</th>
            ...
            <tbody>
              {filteredSales.map((sale, idx) => (
                <tr key={sale.saleId || idx} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '12px 16px', fontWeight: 500, color: '#0f172a' }}>{sale.eventTitle || 'N/A'}</td>
                  <td style={{ padding: '12px 16px', color: '#475569' }}>
                    <div>{sale.userName || 'N/A'}</div>
                    <div style={{ fontSize: '0.8rem', color: '#94a3b8' }}>{sale.userEmail || ''}</div>
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{ background: '#eff6ff', color: '#0a5db8', padding: '4px 10px', borderRadius: 12, fontSize: '0.85rem', fontWeight: 500 }}>
                      {sale.ticketCategory || 'N/A'}
                    </span>
                  </td>
                  <td style={{ padding: '12px 16px', textAlign: 'center', fontWeight: 600 }}>{sale.ticketCount || 0}</td>
